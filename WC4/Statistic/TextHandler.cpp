...
TextHandler::TextHandler(const char* filename) : filename(filename) {
    ...
    this->wordStatistics = new std::map<std::string, int>;
    this->quantity = 0;
    this->wordCount = 0;
}
...
long long TextHandler::getWordCount() const {
...
void TextHandler::parseText() {
    ...
    for ( ; file.get(symbol) ; ) {
        ...
        if ( word != "" && wordEndSymbols.count(symbol) ) {
            this->insert(word, words);
            this->insert(word, wordStatistics);
            word = "";
            this->wordCount += 1;
        }
    }
...
std::ostream& operator<<(std::ostream& out, const TextHandler& handler) {
    ...
    out << "Word statistics:" << handler.getWordCount() << std::endl;
    out << "Unique words" << handler.getWords() << std::endl;
    out << "matches in text: " << std::endl;
